    ...
    <div class="container">
         <div class="row align-items-center mx-0 my-5 d-flex justify-content-center">
             <div class="venue col-md-6 col-12 ">
                 <div class="ven">
                     <form>
                         <h4 class="col-md-12 text-center ">Update Venue</h4>
...
export default {
    ...
    setup() {
        ...
        const fetchVenueDataById = async () => {
            const response = await fetch(`${config.url}/api/get_venue_by_id`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'x-access-tokens': token
        ...
        const handleSubmit = async() => {
            ...
            try {
                const response = await fetch(`${config.url}/api/update_venue`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
            ...
            if (!response.ok) {
                ...
                toast.success('Venue Updated Successfully!',{autoClose: 1000});
        ...
        onUpdated(() => {
            for (let i = 0; i < venue.venues.length; i++) {
                if (venue.venues[i].name == name.value && venue.venues[i].name != mount_name.value) {
